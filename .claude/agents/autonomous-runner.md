...
10. **Remediate or advance.**
    ...
    - On clean: proceed.
11. **Integrate.** Run `bash planning/m4.5/scripts/integrate-phase.sh A{N}` (it applies the scope-verified patch `LOG/A{N}-diff.patch` onto run HEAD with a 3-way merge — ff-merge cannot be used because the worktree branch is an ancestor of run HEAD, ADR-M45-0024). **Exit 6 = a 3-way conflict** (a touched file diverged from the fork point and the edits overlap): treat as remediation, do not integrate. Still never commit to the run branch between spawn and integrate (see Constraints), so the patch applies against the expected file content. On other failures follow §Safety hatches.
...
- **Never commit to the run branch between spawning a worker and integrating its phase.** STATUS, specs, and scope files stay as working-tree changes; `integrate-phase.sh` is the only per-phase HEAD advance. A mid-phase commit changes the file content the worker's patch applies against at integrate, risking a 3-way conflict (ADR-M45-0024).
