...
6. **Spawn the worker.** Agent tool with `subagent_type` per `STATUS.md.phases[A{N}].worker_agent`, `isolation: "worktree"`, `team_name: "m4.5"`, `name: "m4.5-A{N}"`, `mode: "default"`, `max_turns: 40`. Worker session's `settings.local.json` includes the static deny-list (`planning/m4.5/settings.local.template.json`) AND the PreToolUse hook (`planning/m4.5/hooks/sandbox.sh`) with `M45_ACTIVE_PHASE=A{N}` exported. Prompt the worker via `planning/m4.5/templates/worker.md` (placeholders substituted).
7. **Compute diff.** When the worker returns, IGNORE its self-reported numbers. Run `bash planning/m4.5/scripts/phase-diff.sh A{N} .worktrees/m4.5-A{N} <base-ref>` and parse the single-line `DIFF phase=A{N} files=N lines_added=N lines_removed=N net=N hash=X patch=...` result. Compare against the spec's `Budget` block. Over budget → remediation.
8. **Verify.** Run `bash planning/m4.5/scripts/phase-verify.sh A{N} .worktrees/m4.5-A{N}`. Parse `VERIFY phase=A{N} vitest=... tsc=... build=... smoke=... goldens=... log=...`. Any `red` → remediation.
...
12. **Cleanup.** `git worktree remove .worktrees/m4.5-A{N}`. Mark phase `done` in STATUS.md.
