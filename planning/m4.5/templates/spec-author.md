...
- **Preserve existing test mock/DI seams.** When a phase changes an import graph (removes or inverts a dependency), find the tests that rely on the *old* graph — especially `vi.mock(...)` calls that intercept a **transitive** import. Such a mock silently stops working once the import path changes: behavior is dropped and tests fail with no in-scope fix, a guaranteed STALL. Before sealing, either (a) keep the seam working (e.g. a default implementation registered at module load so the mock still intercepts), or (b) bring the affected test files into `Scope: touch`, bump Budget, and prescribe the exact helper/mock update. Name the seam and the chosen approach in the spec. **Check that every Accept assertion is achievable without editing a file outside the touch list** — a spec whose Accept gates contradict its own scope is a defect, not a worker problem.
