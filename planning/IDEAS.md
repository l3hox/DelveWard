# DelveWard — Ideas Parking Lot

Unplanned feature ideas and wishlist items. Not scoped or scheduled — just captured for future reference. See `FEATURES-V2.md` for the prioritized/tiered feature list.

---

- Animated water + lava tiles with lowered floor
- Outdoor sections — no ceiling, skybox, fullbright lighting only for outdoor tiles; indoor/outdoor boundary wall acts as strong light source; day/night cycle affects outdoor light level
- Outdoor forest environment — solid "forest" charDef cells rendered as billboard tree sprites (1-3 per cell, randomly placed/scaled pixelart trees) in front of a green forest-wall texture. Walkable clearings use grass/dirt floor textures. Layered depth: front trees larger/darker, back trees smaller/lighter, dense fog-green wall behind. Stone walls visible through tree gaps when forest cells are adjacent to dungeon walls — ruins peeking through trees. Combine with cemetery theme: graveyard walls, iron fences (using thin walls when available), tombstone entities. Natural habitat for skeleton/zombie encounters. Procedural tree placement per cell (seeded by col/row for determinism).
- Multiple dungeon layers visible at once (vertical openness)
- Footstep screen bob — subtle camera vertical bounce on each step
- Damage directional indicator — brief red arrow on HUD showing attack direction
- Breakable / cracked walls — attack to reveal hidden rooms
- Trap tiles — spikes, darts triggered by stepping on a cell
- Message popups — stone tablets / signs showing lore/hints on interact
- Minimap fog-of-war fade — explored but not visible cells draw dimmer
- Procedural dungeon generation
- Ambient sound — dripping water, distant echoes
- Secret walls that push open
- Throwing items — toss potions/rocks at enemies from range with arc animation
- Environmental hazards — poison gas clouds draining HP while standing in them
- Treasure chests — interact to open, drop random loot, animated lid
- Torch wall shadows — dynamic shadow planes behind pillars/door frames
- ~~Status effects — poison (tick damage), slow (longer tween), blind (reduced torch range)~~ — implemented in M2 Phase C
- ~~Pushable blocks — grid-aligned blocks for dungeon puzzles~~ — implemented in M2 Phase D
- ~~Thin walls — walls between two walkable cells (edge walls, not cell walls), including thin wall doors; enables villages and walk-in houses~~ — implemented in M4 Phase C
- Editor: monster database editor — edit enemy stats (hp, atk, def, aggro, speed, xp), add/remove enemy types, preview sprites, live-save to enemyTypes
- Editor: item database editor — edit items.json directly from the editor, add/remove items, edit stats/modifiers/requirements, icon picker from available sprites
- Editor: direct file save via Vite dev server — add a `configureServer` plugin to `vite.config.ts` exposing `POST /api/write-file` and `GET /api/read-file` routes. Editor writes JSON files (items, enemies, dungeons) directly to the repo during `npm run dev`. Hot-reload picks up changes instantly. In non-dev mode (static build), save is unavailable but export (download) still works as fallback. If public access is ever needed, revisit with a proper backend.
- Quest objective auto-tracking — current quest system is dialog-driven only (advance via NPC dialog effects). No automatic objective evaluation. Future: add objective types (`collectItems`, `killEnemy`, `reachArea`) to quest stage definitions with auto-evaluation. `collectItems`: track item counts in backpack, auto-complete when threshold met. `killEnemy`: listen for enemy kill events, increment counter per enemy type, flag-based or counter-based. `reachArea`: trigger on player entering a cell/zone. HUD objective counter overlay showing "Kill rats: 2/5" or "Collect herbs: 1/3". Objectives would auto-advance quest stages without requiring NPC dialog, enabling more dynamic quest flows. Current dialog-condition-based approach (`hasItem`, `hasFlag`) still works as a fallback for simpler quests.
- Entity dormancy system — spatial data structure (quadtree/octree) for simulation radius around player. Only simulate entities (AI, signals, projectiles, status effects) within a configurable radius, both horizontal and vertical. Entities outside radius are dormant — exist in Maps but skipped in update loops. Needed for large open-world levels with many layers. Geometry always rendered regardless. Separate from rendering culling — purely a simulation optimization. Design as a standalone system that wraps existing entity update loops.
- Quest navigation HUD — directional arrow/indicator on the HUD pointing toward the active quest objective (like modern game waypoint markers). Requires quest stages to define a target location (entity ID, cell coordinates, or NPC). Use BFS/pathfinding on the grid to compute direction from player to target (reuse existing pathfinding module). Render as a compass-style arrow or screen-edge chevron. Should handle cross-level targets (show arrow toward stairs leading to target level). Optional per-quest — some quests (exploration, secrets) should not have a waypoint. Toggle on/off in quest log.
