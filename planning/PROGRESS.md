...
- **Phase D: Environment Entities**: 5 new entity types — breakable_wall (HP-based, attackable, drops loot), secret_wall (walk-into opens passage, `persistent` flag for illusionary walls that stay visible), block (pushable, activates pressure plates, blocks projectiles), chest (open/locked/signal-controlled, loot drops, key-locked), sign (wall-mounted, parchment overlay popup). Wall entity renderer (shared geometry for breakable+secret walls, wallGroup/floorCeilGroup visibility toggle on destroy/open). Dungeon builder `wallEntityCells` skip parameter. Combat extended with wall_hit result type. Player `onMoveBlocked` callback for secret wall detection. Block renderer with push animation. Chest renderer with lid pivot animation. Sign renderer (wall-mounted plane). SignOverlay (DOM overlay with capture-phase input blocking). Interaction extended: block_pushed, chest_opened, chest_locked, sign_read. Level loader validation for all 5 types. Editor: all 5 in entity palette/grid canvas, inspector fields (hp, persistent/illusionary, state, keyId, gateMode, wall, text), placement rules with user-friendly hints, wire map extended for chest targets, sign empty-text warning, "CharDefs" renamed to "Custom Tiles" in UI. 658 tests.
