...
- **Phase A: Signal System Foundation**: Complete M2 signal system. Migrated lever/plate `target: string` → `targets: string[]` (multi-target support). New `SignalManager` class with centralized propagation, cycle detection, and receiver-changed callbacks. Signal modes: toggle, momentary, one_shot, timed. Optional `signalDelay` (checkbox + seconds) composes with any mode — delays activation by N seconds. Gate modes on doors: or, and, xor. Three new entity types: trigger (invisible floor trigger), tripwire (one-shot with orientation auto-detect and visibility threshold), standalone gate (and/or/not/delay/pulse_edge/pulse_repeat). Sources can target both doors and gates. Full editor support: toolbar buttons for all new types, inspector fields with labeled signal mode dropdown (tooltips), targets array with clickable IDs, signal delay checkbox. Grid icons: dashed circle (trigger), oriented dashed line with end dots (tripwire), diamond with gate-type label (gate). Wiring arrows from all targets-array entities. Auto-detect wall facing on lever/sconce placement, auto-detect tripwire orientation. New entity placement copies targets from selected entity of same type. Entity deletion cleans up all references. Level loader uses soft entity validation — invalid entities are warned and skipped for game, not fatal. Editor export/save no longer gated on errors. `onDoorSignalChanged` callback wires signal-driven door state to mesh animation. DUNGEON-DESIGNER.md updated with all new entity docs. 569 tests passing.
