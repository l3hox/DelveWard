...
- **Phase A: Signal System Foundation + Editor Polish + Signal Behavior Fixes**: Complete M2 signal system with full editor integration. Core: `targets: string[]` migration, `SignalManager` (propagation, cycle detection, gate evaluation), signal modes (toggle/momentary/one_shot/timed), `signalDelay`, gate modes on doors (or/and/xor). New entities: trigger, tripwire (with orientation), standalone gate. Editor: toolbar icons, inspector fields (signal mode with tooltips, targets with clickable `type @ (col, row)` refs, signal delay checkbox), grid icons, wiring arrows with full signal chain highlighting. Selecting any entity highlights entire chain (forward + backward). Signal loop detection in editor validation with immediate feedback on wire/delete. "Referenced by" section on doors and gates with remove buttons. Auto-detect wall/orientation on placement, target copy from selected entity, reference cleanup on delete. Deep-copy entity defaults. Soft entity validation (game skips invalid entities, editor warns). Unified clickable entity link format (`type @ (col, row)`, blue `#88aaff`) across inspector and error banner. Inspector hover highlights entity on grid canvas (blue overlay) and cross-level refs highlight target level in list. Signal state persisted in LevelSnapshot (fixes AND/XOR gates breaking after level transition). Lever signal modes restricted to toggle/one_shot/timed (no momentary). Timed levers auto-reset with animation. Pressure plate modes: toggle (flips on/off each step), momentary (visual feedback on step-off), timed (countdown starts on step-off). Door blocking: signal-driven doors bounce open on occupied cells, retry every 1.5s until clear. Door panel edge UV fix. 572 tests.
