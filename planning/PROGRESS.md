...
- **Phase A: Signal System Foundation**: Complete M2 signal system. Migrated lever/plate `target: string` → `targets: string[]` (multi-target support). New `SignalManager` class with centralized propagation, cycle detection, and receiver-changed callbacks. Signal modes: toggle, momentary, one_shot, timed. Gate modes on doors: or, and, xor. Three new entity types: trigger (invisible floor trigger, momentary/one_shot), tripwire (one-shot with visibility threshold), standalone gate (and/or/not/delay/pulse_edge/pulse_repeat). Full editor support: inspector fields for all signal properties, grid icons (dashed circle, dashed line, diamond), wiring arrows from all targets-array entities. Level validation for all new fields and entity types. DUNGEON-DESIGNER.md updated. 564 tests passing.
